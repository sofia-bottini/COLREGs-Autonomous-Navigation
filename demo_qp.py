...
def main():
    ...
    env_params_channel.set_float_parameter("eval_episode_seed", 15.0) #try with: 15, 55, -300000
    ...
    metrics = MetricsTracker(d_safe=2.5) # Aligned to the hull's true bubble

    # 4 GLOBAL BUOYS WITH REAL COORDINATES FROM UNITY
    global_buoys = [
        np.array([0.04, 5.16]),   # Boa 1
        np.array([0.078, -4.99]), # Boa 2
        np.array([5.07, 0.04]),   # Boa 3
        np.array([-4.95, 0.15])   # Boa 4
    ]
            ...
            if len(decision_steps) > 0:
                ...
                for sensor_data in decision_steps.obs:
                    if sensor_data.shape[1] >= 20: 
                        obs_gps = sensor_data[0] 
                    elif sensor_data.shape[1] == 14:
                        obs_ray = sensor_data[0] 
